...
class BayesClassifier:
    ...
    def __init__(self):
        ...
        if os.path.isfile(self.pos_filename) and os.path.isfile(self.neg_filename):
            print("Data files found - loading to use cached values...") #loading the dictionaries and saving them to pos & neg
    ...
    def train(self) -> None:
        ...
        #     <the rest of your code for updating frequencies here>
            ...
            text = self.load_file(os.path.join(self.training_data_directory, filename)) #we've now extracted the text
            # print(text)
            tokens = self.tokenize(text) #takes the individual words and puts them in a list (all lowercase)
            # print(tokens)
        ...
        self.save_dict(self.pos_freqs, self.pos_filename) #both parameters are attributes
    ...
    def classify(self, text: str) -> str:
        ...
        token_pos_score = sum(self.pos_freqs.values())
        token_neg_score = sum(self.neg_freqs.values())
        ...
        for token in tokens:
            ...
            # print(f"{token} has a negative freq of {neg_token_freq}")
            pos_score += math.log(pos_token_freq / pos_total)
            token_pos_score = math.log(pos_token_freq / pos_total)
            # print(f"{token} has a positive score of {token_pos_score}") # wasn't initially summing the pos_score or neg_score
